...
export default class HostMonitorRemove extends Command {
  static readonly description = 'Remove the monitor from a host (script + env + crontab block + relay host)';
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(HostMonitorRemove);
    try {
