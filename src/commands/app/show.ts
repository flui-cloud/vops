...
export default class AppShow extends Command {
  ...
  async run(): Promise<void> {
    ...
    await runAgentCommand(
      ...
      flags.json,
      async () => ({
        data: await withService(VopsAppsService, (svc) => svc.show(args.name)),
        nextActions: [
          { command: `vops app status ${args.name} --json`, description: 'Whether it is actually running right now' },
          { command: `vops app logs ${args.name} --json`, description: 'Recent journald output' },
        ],
      }),
      (i) => {
